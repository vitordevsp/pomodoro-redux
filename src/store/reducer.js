...
import {
  newTask, editTask, delTask, changeDoneTask, movingTask,
...
function reducer(state = INITIAL_STATE, action) {
  switch (action.type) {
    ...
    case 'DEL_POMODORO': {
      ...
      const rearrangingArray = (obj, index) => ({ ...obj, indicator: `#${index + 1}` })
    ...
    case 'MOVING_TASK': {
      const {
        indicatorPomodoro, indexTask, objTask, newIndicator,
      } = action.value
      const arrayPomodoros = state.pomodoros
